...
class PlayScraper(object):
    ...
    def _parse_app_details(self, soup):
        ...
        try:
            full_price = soup.select_one('span[jsan="7.full-price"]').string
        except:
            full_price = "Not Available"
            pass

        free = (price == '0' and full_price == "Not Available")
        free_now = (price == '0' and full_price != "Not Available")
        ...
        try:
            installs = [int(n.replace(',', '').replace('.', '')) for n in additional_info.select_one(
    ...
    def _parse_app_details_min(self, soup):
        """Extracts an app's details from its info page.
    ...
    def _parse_multiple_apps(self, list_response):
        ...
        soup = BeautifulSoup(list_response.content, s.PARSER, parse_only=list_strainer)
        ...
        for i, r in enumerate(responses):
            if r is not None and r.status_code == requests.codes.ok:
                soup = BeautifulSoup(r.content, s.PARSER, parse_only=app_strainer)
    ...
    def details(self, app_id):
        ...
        try:
            ...
            soup = BeautifulSoup(response.content, s.PARSER)
    ...
    def details_min(self, app_id):
        ...
        return self._parse_app_details_min(soup)
